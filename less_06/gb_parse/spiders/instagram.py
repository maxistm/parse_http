...
class InstagramSpider(scrapy.Spider):
    ...
    @staticmethod
    def js_add_data_extract(response):
        script = response.xpath(
            '//script[contains(text(), "window.__additionalDataLoaded =")]/text()').get()
        return json.loads(script.replace("window.__additionalDataLoaded =", '')[:-1])
